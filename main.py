...
# в данном случае errors="coerce". Это означает, что в случае ошибки вместо даты в ячейку записывается NaT (что, вроде как,
# означает что-то типа "дата не определена". Пустое значение).
...
# содержащего числовые значения, обеспечивает метод describe. Его одного достаточно для выполнения задания 5, так как
# данный метод выводит не только значения перцентилей (перцентили, о которых мы хотим узнать информацию, указаны в параметре percentiles),
...
# для вывода переменных сразу в предназначенной для вывода текстовой константе (переменные записываются внутри {}) без
# разбиения выводимой строки на 99999
# отельных строковых констант, заключённых в кавычки, чередующихся с переменными, и клоунских запятых.
...
# колонку с жанром с помощью .str.cat(sep=",") собираем в одну строку, значения которой разделны запятыми
# (не совсем знаю, как это работает в подробностях, но, вроде, происходит именно это) и далее разбиваем её на элементы списка
...
# в список только индексы. rotation=90 обеспечивает поворот надписей на 90 градусов (хотя это мало спасает)
...
# тем, что в названии какого-то из анимэ присутствует невидимый pyCharm символ. (ошибка будет называться вроде UserWarning: Glyph 12539)
...
# создаём series под иминем dates, который хранит отсортированные по кол-ву выпущенных аниме года. Для этого
...
# задание 10 в котором у меня вместо количества аниме (как должно быть) рассматривается количество компаний. Но суть решения схожая
list_of_rat = {}#нет, это не список крыс. Это словарь, который каждому ключу-интервалу сопоставляет количество производителей с рейтингом из данного интервала
...
# (то-есть равен искомому жанру) записывается True. BИначе - False.Эта маска обновляется с каждым новым циклом
